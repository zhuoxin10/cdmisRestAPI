...
function schedule() {
  ...
  for (let i = 0; i < counselItem.length; i++) {
    ...
    if (lastTime > deadLine) {
      ...
      let count_of_reply = db.communications
        .find({ sendBy: cmudoc, receiver: cmupat, sendDateTime: { $gt: counselItem[i].time } })
        .toArray().length;
      // 插入科主任、结束时间、超时类型、更改状态为0
      if (departmentItem.length === 0) {
        // 若该医生不存在科室
        db.counselautochangestatuses.update(
          { counselId: counselItem[i].counselId },
          { $set: { status: 0, endTime: now, timeouttype: 1, reply: count_of_reply } }
        );
      } else {
        db.counselautochangestatuses.update(
          { counselId: counselItem[i].counselId },
          {
            $set: {
              departLeader: departmentItem[0].departLeader,
              status: 0,
              endTime: now,
              timeouttype: 1,
              reply: count_of_reply
            }
          }
        );
      }
      ...
      db.accounts.update({ userId: cmupat }, { $pull: { times: { doctorId: cmudoc } } });
      db.accounts.update({ userId: cmupat }, { $push: { times: { doctorId: cmudoc, count: 0 } } });
