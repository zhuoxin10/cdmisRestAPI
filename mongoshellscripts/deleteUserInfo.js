...
if (user.length === 0) {
  ...
  if (userRole.indexOf("patient") !== -1) {
    ...
    query = {
      $or: [{ sendBy: userId }, { receiver: userId, userRole: "patient" }]
    };
    ...
    printjson({
      ...
      model: "counselautochangestatus",
      delete_item: counselautochangestatusResult
    });
  ...
  if (userRole.indexOf("doctor") !== -1) {
    ...
    query = {
      $or: [{ sendBy: userId }, { receiver: userId, userRole: "doctor" }]
    };
    let newResult = db.news.remove(query);
    printjson({ result: "delete_success", model: "new", delete_item: newResult });
    ...
    query = {
      ...
      // 'hospital': workUnit
      $or: [{ portleader: userObjectId }, { doctors: userObjectId }, { departLeader: userObjectId }]
    };
    upObj = {
      $pull: {
    ...
    query = {
      $or: [{ "doctorReport.doctorId": userObjectId }, { "doctorComment.doctorId": userObjectId }]
    };
    upObj = {
      $pull: {
        doctorReport: {
